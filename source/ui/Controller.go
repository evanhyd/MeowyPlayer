...
func newController() fyne.CanvasObject {
	coverViewSize := fyne.NewSize(128.0, 128.0)
	coverView := cwidget.NewCoverView(coverViewSize)
	manager.GetCurrentAlbum().Attach(utility.MakeCallback(func(album *player.Album) {
		coverView.SetAlbum(album)
		coverView.OnTapped = func(*fyne.PointEvent) { manager.GetCurrentAlbum().Set(album) }
	}))

	return container.NewBorder(nil, nil, coverView, nil, cwidget.NewMusicController())
