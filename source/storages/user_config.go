...
func GetUserConfig(baseDir string) UserConfig {
	...
	if configData, err := os.ReadFile(filepath.Join(baseDir, "config.json")); err != nil {
		...
		config.Endpoints = map[string]string{
			...
			"putPlaylistMusic":     `http://147.5.96.83/api/putPlaylistMusic`,
			"putPlaylistMusicBulk": `http://147.5.96.83/api/putPlaylistMusicBulk`,
			"deletePlaylistMusic":  `http://147.5.96.83/api/deletePlaylistMusic`,
		}
